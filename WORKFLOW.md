...
5종은 **사람이 쓴다.** `PROGRESS.md`는 에이전트가 진행하며 쌓는다.
...
`docs/analysis/`는 조사·실측 결과를 담는다. 규칙이 아니라 사실이고, 결정 하나에
묶이지 않으며, 버전이 바뀌어도 그대로다. `CLAUDE.md`에 넣을 분량이 아니고
`DECISIONS.md`에 넣으면 버전마다 복사된다.

`src/`, `scripts/`, `tests/`, `docs/analysis/`는 **첫 필요 시점에 만든다.** 초기화 때
...
| 패턴 | 증상 | 교정 |
| --- | --- | --- |
| 에이전트에게 5종을 쓰게 함 | 요구가 일반론이 되고 판단 기준이 안 됨 | 사람이 작성해 넘김 |
